...
public class Plateau {
    ...
    private boolean aJoue; //utiliser dans Main.deroulementPartieGraphique() pour savoir si le joueur a deja joue
    ...
    Elles construisents un plateau constitue de cases hexagonales remplie avec les differentes couleurs de maniere aleatoire
    (informations sur les couleurs contenues dans le tableau a  2 dimentions 'cases')
    Le premier joueur est place en haut a  gauche, le 2e en bas a  droit
    ...
    */
    ...
    public Plateau(Joueur joueur1, Joueur joueur2,Joueur joueur3, int typeTableau,boolean obstacle, int taille){
        ...
        }//Il faut va©rifier qu'un joueur n'a pas une couleur obstacle
        while(caz[0][0]==6){
            caz[0][0]=rand.nextInt(6);
        }
        this.joueur1.setCouleur(caz[0][0]);
    ...
    // On definit quelques fonctions de bases permettant l'acces a  certaines donnees du jeu ou leur definition/modification
     ...
     *On sait si une case est contra´lee ou non gra¢ce au tableau 'casesControle'
     *La lettre affichee correspond a  l'intitiale de la couleur, en minuscule si la case n'est pas contra´lee, en majuscule sinon
     */
    ...
    Dans ce cas la fonction remet a  jour le tableau de controle du jeu afin d'ajouter les nouvelles conquetes du
    ...
    */
    ...
    /*
    Cette fonction verifie si un joueur controle les cases adjascentes aux siennes (dans le cas ou elles seraient de la meme couleur que sa couleur choisie)
    ...
    Ensuite pour chaque case du tableau, si elle est controlee par le joueur X, on l'ajoute a  cette pile
    ceci constitue la premiere phase d'initialisation de la fonction
    on va ensuite faire appel a  la 2e pertie de la fonction qui se rappellera elle meme jusqu'a  avoir detecte toutes les nouvelles cases controlees par le joueur X
    */
    ...
    */
    ...
    public boolean joueIAGraphique(Joueur joueurX, Enregistreur piFenetre){
        ...
        // obsolete // piFenetre.getPI().repaint(); // on redessine le Graphique
    ...
    */
    ...
    public int comptePointsHypothetiquesJoueurParCouleur(Joueur joueurX,int couleur){
       ...
       (avec la verification de contrôle des cases et l'application de la regle avancee)
    ...
    */
    ...
    /**
     * aucune cases n'a encore ete teste aucun joueurs rencontres...
     * @param dejaPassee
    ...
    /**
     * tres honnetement meme moi je suis pas sur de l'avoir compris celle-la, schema obligatoire. le but etant en la combinant avec versGauche() de passe par
    ...
    private void versDroite (int i, int j, boolean[][] dejaPassee, boolean[] joueursRencontres){
        if(dejaPassee[i][j] == false && (i>=0) && (j>=0) && (i<this.taille) && (j<this.taille) ){ // si on est out of bound inutile et si on est deja passe cela ne sert a rien (excepter lors des changement d'etages ou il faut partir des deux cotes sur la meme case et la on repasse a false avant d'aller versGauche()
            dejaPassee[i][j] = true;
    ...
    private void versGauche (int i, int j, boolean[][] dejaPassee, boolean[] joueursRencontres){
        if(dejaPassee[i][j] == false && (i>=0) && (j>=0) && (i<this.taille) && (j<this.taille) ){ // si on est deja passe cela ne sert a rien (excepter lors des changement d'etages ou il faut partir des deux cotes sur la meme case et la on repasse a false avant d'aller versGauche()
            dejaPassee[i][j] = true;
            int coteTeste = this.coteTeste(i, j-1);
